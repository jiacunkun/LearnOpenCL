...
#define REPORT_LOG_IF(severity, condition) \
    !(condition) ? (void) 0 : acv::internal::VoidLogger() & \
      acv::internal::Logger((char *)__FILE__, __LINE__, severity).stream()
  

#define REPORT_LOG_FATAL                REPORT_LOG_IF(acv::FATAL, acv::FATAL<=MAX_LOG_LEVEL)
#define REPORT_LOG_IF_FATAL(condition)  REPORT_LOG_IF(acv::FATAL, (acv::FATAL<=MAX_LOG_LEVEL)&&(condition))


#define REPORT_LOG_ERROR                REPORT_LOG_IF(acv::AERROR, acv::AERROR<=MAX_LOG_LEVEL)
#define REPORT_LOG_IF_ERROR(condition)  REPORT_LOG_IF(acv::AERROR, (acv::AERROR<=MAX_LOG_LEVEL)&&(condition))


#define REPORT_LOG_WARNING              REPORT_LOG_IF(acv::WARNING, (acv::WARNING<=MAX_LOG_LEVEL))
#define REPORT_LOG_IF_WARNING(condition) REPORT_LOG_IF(acv::WARNING, (acv::WARNING<=MAX_LOG_LEVEL)&&(condition))


#define REPORT_LOG_INFO                REPORT_LOG_IF(acv::INFO, (acv::INFO<=MAX_LOG_LEVEL))
#define REPORT_LOG_IF_INFO(condition)  REPORT_LOG_IF(acv::INFO, (acv::INFO<=MAX_LOG_LEVEL)&&(condition))

#define REPORT_LOG_DEBUG                REPORT_LOG_IF(acv::DEBUG, (acv::DEBUG<=MAX_LOG_LEVEL))
#define REPORT_LOG_IF_DEBUG(condition)  REPORT_LOG_IF(acv::DEBUG, (acv::DEBUG<=MAX_LOG_LEVEL)&&(condition))
...
namespace acv {
	...
	T* CheckPtrNotNull(T* t, const char* file, int line, const char* message) {
		if (t == NULL) {
			internal::Logger(file, line, FATAL).stream() << message;
		}
		return t;
	}
...
#define CHECK_NOTNULL(val) \
  acv::CheckPtrNotNull((val), __FILE__, __LINE__, "'" #val "' must be non NULL" )
