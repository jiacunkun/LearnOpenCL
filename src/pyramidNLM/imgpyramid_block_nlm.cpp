                                        ...
                                        MFloat fNoiseVarY, MFloat fNoiseVarUV, LPASVLOFFSCREEN pMaskImg, MFloat lISO)
{
    ...
    MInt32  lLevel = 4;
    MInt32  lL;
    Img_Plane_Data Mask_Data[5] = { MNull };
    Img_Plane_Data Y_Data[4] = { MNull };
    Img_Plane_Data UVData[8] = { MNull };
    Img_Plane_Data TmpDstY = { MNull }, TmpDstU = { MNull }, TmpDstV = { MNull };
    Img_Plane_Data BigDstU = { MNull }, BigDstV = { MNull };
    ...
    Img_Plane_Data UExpResule = { MNull };
    Img_Plane_Data VExpResule = { MNull };
    Img_Plane_Data YGuideImg = { MNull };
    Img_Plane_Data UVGuideImg = { MNull };
    ...
    lret = MaskImage_Pyramid_Creat_Build(hMemMgr, mcvParallelMonitor, pMaskImg, Mask_Data, lLevel + 1);
    if (MOK != lret)
    {
        goto exit;
    }
...
#endif
    //The NLM Process from lLevel - 1 to 1
    //The Biggest Layer dose not do the NLM denoise.
    for (lL = lLevel - 1; lL > 0; lL--)
    {
        MFloat fTmpVarY = fNoiseVarY * pow(0.5, lL);
        MFloat fTmpVarUV = fNoiseVarUV * pow(0.5, lL);
        ...
        if (bY_Flag)
        {
            ...
            if (lL == lLevel - 1)
            {
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &Y_Data[lL], &TmpDstY, &Mask_Data[lL], pMapY, pInvMap);
            }
            else
            {
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &YExpResule, &TmpDstY, &Mask_Data[lL], pMapY, pInvMap);
            }
#ifdef _OUTPUT_LOG_
            {
				char szFile[256];
				sprintf(szFile, "%s/Single/SingleNLM_%d.bmp", OUTPUT_PATH, lL);
				SaveToBMP(szFile, TmpDstY.pImage, TmpDstY.lWidth, TmpDstY.lHeight, TmpDstY.lPitch, 8);
			}
#endif
            if (MOK != lret)
            {
                ...
                {
				MChar szName[260];
				sprintf(szName, "%s/Single/YImage_Laver_%d_nl.bmp", OUTPUT_PATH, lL);
				SaveToBMP(szName, TmpDstY.pImage, TmpDstY.lWidth, TmpDstY.lHeight, TmpDstY.lPitch, 8);
			}
#endif
            Img_Sub_C1(mcvParallelMonitor, &TmpDstY, &Y_Data[lL]);
            ...
            if (1 == lL)
            {
                ...
                {
					char szFile[256];
					sprintf(szFile, "%s/Single/NLM_Detail_%d_before.bmp", OUTPUT_PATH, lL);
					SaveToBMP(szFile, Y_Data[lL - 1].pImage, Y_Data[lL - 1].lWidth, Y_Data[lL - 1].lHeight, Y_Data[lL - 1].lPitch, 8);
					MFloat t = 0;
				}
#endif
                MakeWeightMap_Block(pMapY, fNoiseVarY*0.6, 16 * 50);
                //Img_Add_C1(mcvParallelMonitor, &Y_Data[lL - 1], &YExpResule);
                Img_Add_C1(mcvParallelMonitor, &YExpResule, &Y_Data[lL - 1]);
                ...
                {
					char szFile[256];
					sprintf(szFile, "%s/Single/NLM_Detail_%d.bmp", OUTPUT_PATH, lL);
					SaveToBMP(szFile, YExpResule.pImage, YExpResule.lWidth, YExpResule.lHeight, YExpResule.lPitch, 8);
				}
#endif
                lret = Local_ImgData(hMemMgr, &YGuideImg, YExpResule.lWidth, YExpResule.lHeight, YExpResule.lPitch, 1);
                if (MOK != lret)
                {
                    goto exit;
                }
                lret = Cross_Median_Filter_5x5(hMemMgr, mcvParallelMonitor, YExpResule.pImage, YExpResule.lWidth, YExpResule.lHeight, YExpResule.lPitch,
                                               YGuideImg.pImage, YGuideImg.lPitch);
                if (MOK != lret)
                {
                    goto exit;
                }

                lret = Img_Denoise_Guide_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &YExpResule, &Y_Data[lL - 1], &YGuideImg, &Mask_Data[0], pMapY, pInvMap);
                if (MOK != lret)
                {
                    goto exit;
                }
                ...
                {
					char szFile[256];
					sprintf(szFile, "%s/Single/NLM_Dst_%d.bmp", OUTPUT_PATH, lL-1);
					SaveToBMP(szFile, Y_Data[lL - 1].pImage, Y_Data[lL - 1].lWidth, Y_Data[lL - 1].lHeight, Y_Data[lL - 1].lPitch, 8);
				}
#endif
			...
			{
				MChar szName[260];
				sprintf(szName, "%s/Single/YImage_Laver_%d_ex.bmp", OUTPUT_PATH, lL - 1);
				SaveToBMP(szName, Y_Data[lL - 1].pImage, Y_Data[lL - 1].lWidth, Y_Data[lL - 1].lHeight, Y_Data[lL - 1].lPitch, 8);
			}
			else
			{
            ...
            Free_ImgData(hMemMgr, &Y_Data[lL]);
            Free_ImgData(hMemMgr, &TmpDstY);
            Free_ImgData(hMemMgr, &YGuideImg);
        }
        //do UV
        if (bUV_Flag)
        {
            ...
            if (lL == lLevel - 1)
            {
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &UVData[lL * 2], &TmpDstU, &Mask_Data[lL + 1], pMapUV, pInvMap);
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &UVData[lL * 2 + 1], &TmpDstV, &Mask_Data[lL + 1], pMapUV, pInvMap);
            }
            else
            {
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &UExpResule, &TmpDstU, &Mask_Data[lL + 1], pMapUV, pInvMap);
                lret = Img_Denoise_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &VExpResule, &TmpDstV, &Mask_Data[lL + 1], pMapUV, pInvMap);
            }
            if (MOK != lret)
            {
                goto exit;
                ...
                {
				MChar szName[260];
				sprintf(szName, "%s/Single/UImage_Laver_%d_nl.bmp", OUTPUT_PATH, lL);
				SaveToBMP(szName, TmpDstU.pImage, TmpDstU.lWidth, TmpDstU.lHeight, TmpDstU.lPitch, 8);

				sprintf(szName, "%s/Single/VImage_Laver_%d_nl.bmp", OUTPUT_PATH, lL);
				SaveToBMP(szName, TmpDstV.pImage, TmpDstV.lWidth, TmpDstV.lHeight, TmpDstV.lPitch, 8);
			}
#endif
            ...
            {
                //���Ҳ��һ��NLM
                MakeWeightMap_Block(pMapUV, fNoiseVarUV*0.6, 16 * 50);
                lret = Local_ImgData(hMemMgr, &BigDstU, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch, 1);
                lret = Local_ImgData(hMemMgr, &BigDstV, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch, 1);
                if (MOK != MOK)
                {
                ...
                lret = Local_ImgData(hMemMgr, &UVGuideImg, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch, 1);
                if (MOK != lret)
                {
                    goto exit;
                }
                ...
                //if (fNoiseVarUV <= 20)
                {
                    lret = Cross_Median_Filter_5x5(hMemMgr, mcvParallelMonitor, UExpResule.pImage, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch,
                                                   UVGuideImg.pImage, UVGuideImg.lPitch);
                    if (MOK != lret)
                    {
                        goto exit;
                    }
                    lret = Img_Denoise_Guide_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &UExpResule, &BigDstU, &UVGuideImg, &Mask_Data[1], pMapUV, pInvMap);

                    lret = Cross_Median_Filter_5x5(hMemMgr, mcvParallelMonitor, VExpResule.pImage, VExpResule.lWidth, VExpResule.lHeight, VExpResule.lPitch,
                                                   UVGuideImg.pImage, UVGuideImg.lPitch);
                    if (MOK != lret)
                    {
                        goto exit;
                    }
                    lret = Img_Denoise_Guide_Block_Mask_NLM(hMemMgr, mcvParallelMonitor, &VExpResule, &BigDstV, &UVGuideImg, &Mask_Data[1], pMapUV, pInvMap);
                }

#ifdef _OUTPUT_LOG_
                {
					MChar szName[260];
					sprintf(szName, "%s/Single/UImage_Laver_%d_dst.bmp", OUTPUT_PATH, lL - 1);
					SaveToBMP(szName, BigDstU.pImage, BigDstU.lWidth, BigDstU.lHeight, BigDstU.lPitch, 8);

					sprintf(szName, "%s/Single/VImage_Laver_%d_dst.bmp", OUTPUT_PATH, lL - 1);
					SaveToBMP(szName, BigDstV.pImage, BigDstV.lWidth, BigDstV.lHeight, BigDstV.lPitch, 8);
				}
#endif

                //else
                //{
                //	lret = Local_ImgData(hMemMgr, &HalfSrc, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch);
                //	if (MOK != lret)
                //	{
                //		goto exit;
                //	}
                ...
                //		goto exit;
                //	}
                //	lret = GuidedFilter_Mask_C1_C2(hMemMgr, mcvParallelMonitor, UExpResule.pImage, VExpResule.pImage, BigDstU.pImage, BigDstV.pImage,
                //		UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch, Mask_Data[1],
                //		HalfSrc.pImage, UExpResule.lPitch, fISO);
                //	if (MOK != lret)
                //	{
                //		goto exit;
			...
			{
				MChar szName[260];
				sprintf(szName, "%s/Single/UImage_Laver_%d_ex.bmp", OUTPUT_PATH, lL - 1);
				SaveToBMP(szName, UExpResule.pImage, UExpResule.lWidth, UExpResule.lHeight, UExpResule.lPitch, 8);

				sprintf(szName, "%s/Single/VImage_Laver_%d_ex.bmp", OUTPUT_PATH, lL - 1);
				SaveToBMP(szName, VExpResule.pImage, VExpResule.lWidth, VExpResule.lHeight, VExpResule.lPitch, 8);
			}
			else
			{
				MChar szName[260];
				sprintf(szName, "%s/Single/UVImage_Laver_%d_ex.bmp", OUTPUT_PATH, lL - 1);
				SaveToBMP(szName, UVData[0].pImage, UVData[0].lWidth * 2, UVData[0].lHeight, UVData[0].lPitch, 8);
            ...
            Free_ImgData(hMemMgr, &TmpDstU);
            Free_ImgData(hMemMgr, &TmpDstV);
            Free_ImgData(hMemMgr, &UVGuideImg);
            if (1 == lL)
            {
                Free_ImgData(hMemMgr, &BigDstU);
                Free_ImgData(hMemMgr, &BigDstV);
    ...
    Free_Image_Pyramid_Mem(hMemMgr, Mask_Data, MNull, lLevel + 1);
