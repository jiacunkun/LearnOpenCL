...
{
#ifdef _SSE_LOG_
	printf("Enter Odd_Line_UpScaele_C1\n");
#endif
	...
	const int permlo = 0 | (2 << 4);
	const int permhi = 1 | (3 << 4);
	__m256i* pvu8_half0 = (__m256i*)pHalfLine;
	__m256i* pvu8_half1 = (__m256i*)(pHalfLine + 1);
	__m256i* pvu8_full = (__m256i*)(FullLine + k);
	for (; x < lHalfWidth - 31; x += 32, k += 64) {
		__m256i vu8_half0 = _mm256_loadu_si256(pvu8_half0++);
		__m256i vu8_half1 = _mm256_loadu_si256(pvu8_half1++);

		__m256i vu8_even = _mm256_avg_epu8(vu8_half0, vu8_half1);
		__m256i vu8_odd = vu8_half1;
		__m256i vu8_dst0 = _mm256_unpacklo_epi8(vu8_even, vu8_odd);
		__m256i vu8_dst1 = _mm256_unpackhi_epi8(vu8_even, vu8_odd);

		_mm256_storeu_si256(pvu8_full++, _mm256_permute2x128_si256(vu8_dst0, vu8_dst1, permlo));
		_mm256_storeu_si256(pvu8_full++, _mm256_permute2x128_si256(vu8_dst0, vu8_dst1, permhi));
	}
#elif defined _SSE_OPT_
	__m128i* pvu8_half = (__m128i*)pHalfLine;
	__m128i* pvu8_full = (__m128i*)(FullLine + k);
	__m128i vu8_half0 = _mm_loadu_si128(pvu8_half++);
	for (; x < lHalfWidth - 15; x += 16, k += 32) {
		__m128i vu8_half1 = _mm_loadu_si128(pvu8_half++);
		__m128i vu8_align = _mm_alignr_epi8(vu8_half1, vu8_half0, 1);

		__m128i vu8_even = _mm_avg_epu8(vu8_half0, vu8_align);
		__m128i vu8_odd = vu8_align;
		__m128i vu8_dst0 = _mm_unpacklo_epi8(vu8_even, vu8_odd);
		__m128i vu8_dst1 = _mm_unpackhi_epi8(vu8_even, vu8_odd);

		_mm_storeu_si128(pvu8_full++, vu8_dst0);
		_mm_storeu_si128(pvu8_full++, vu8_dst1);
		vu8_half0 = vu8_half1;
	}
#endif
	for (; x < lHalfWidth; x++, k+=2)
	{
...
#ifdef _SSE_LOG_
	printf("Leave Odd_Line_UpScaele_C1\n");
#endif
	return;
}
...
{
#ifdef _SSE_LOG_
	printf("Enter Even_Odd_Line_UpScaele_C1\n");
#endif
	...
	__m256i* pvu8_half0 = (__m256i*)pHalfLine;
	__m256i* pvu8_half1 = (__m256i*)(pHalfLine + 1);
	__m256i* pvu8_pre = (__m256i*)PreFullLine;
	__m256i* pvu8_nex = (__m256i*)NexFullLine;
	__m256i* pvu8_cur = (__m256i*)CurFullLine;

	for (; x < lHalfWidth - 32; x += 32, k += 64) {
		__m256i vu8_half0 = _mm256_loadu_si256(pvu8_half0++);
		__m256i vu8_half1 = _mm256_loadu_si256(pvu8_half1++);
		__m256i vu8_pre0 = _mm256_loadu_si256(pvu8_pre++);
		__m256i vu8_pre1 = _mm256_loadu_si256(pvu8_pre++);

		__m256i vu8_even = vu8_half0;
		__m256i vu8_odd = _mm256_avg_epu8(vu8_half0, vu8_half1);

		__m256i vu8_tmp0 = _mm256_unpacklo_epi8(vu8_even, vu8_odd);
		__m256i vu8_tmp1 = _mm256_unpackhi_epi8(vu8_even, vu8_odd);
		__m256i vu8_next0 = _mm256_permute2x128_si256(vu8_tmp0, vu8_tmp1, permlo);
		__m256i vu8_next1 = _mm256_permute2x128_si256(vu8_tmp0, vu8_tmp1, permhi);

		_mm256_storeu_si256(pvu8_nex++, vu8_next0);
		_mm256_storeu_si256(pvu8_nex++, vu8_next1);

		__m256i vu8_cur = _mm256_avg_epu8(vu8_next0, vu8_pre0);
		_mm256_storeu_si256(pvu8_cur++, vu8_cur);
		vu8_cur = _mm256_avg_epu8(vu8_next1, vu8_pre1);
		_mm256_storeu_si256(pvu8_cur++, vu8_cur);
	}
#elif defined _SSE_OPT_
	__m128i* pvu8_half = (__m128i*)pHalfLine;
	__m128i* pvu8_pre = (__m128i*)PreFullLine;
	__m128i* pvu8_nex = (__m128i*)NexFullLine;
	__m128i* pvu8_cur = (__m128i*)CurFullLine;

	__m128i vu8_half0 = _mm_loadu_si128(pvu8_half++);
	for (; x < lHalfWidth - 16; x += 16, k += 32) {
		__m128i vu8_half1 = _mm_loadu_si128(pvu8_half++);
		__m128i vu8_pre0 = _mm_loadu_si128(pvu8_pre++);
		__m128i vu8_pre1 = _mm_loadu_si128(pvu8_pre++);

		__m128i vu8_align = _mm_alignr_epi8(vu8_half1, vu8_half0, 1);

		__m128i vu8_even = vu8_half0;
		__m128i vu8_odd = _mm_avg_epu8(vu8_half0, vu8_align);
		__m128i vu8_next0 = _mm_unpacklo_epi8(vu8_even, vu8_odd);
		__m128i vu8_next1 = _mm_unpackhi_epi8(vu8_even, vu8_odd);

		_mm_storeu_si128(pvu8_nex++, vu8_next0);
		_mm_storeu_si128(pvu8_nex++, vu8_next1);

		__m128i vu8_cur = _mm_avg_epu8(vu8_next0, vu8_pre0);
		_mm_storeu_si128(pvu8_cur++, vu8_cur);
		vu8_cur = _mm_avg_epu8(vu8_next1, vu8_pre1);
		_mm_storeu_si128(pvu8_cur++, vu8_cur);
		vu8_half0 = vu8_half1;
	}
#endif
...
#ifdef _SSE_LOG_
	printf("Leave Even_Odd_Line_UpScaele_C1\n");
#endif
	return;
}


MInt32 Img_UpScale2_C1_Range(MHandle hMemMgr, LPImgPyramid_Block_NLM pHalfImg, LPImgPyramid_Block_NLM pFullImg, MInt32 lTopLine, MInt32 lBotLine)
{
	MInt32 lret = MOK;
...
typedef struct _tag_IMG_UP_SCALE_ST{
	MHandle hMemMgr;
	LPImgPyramid_Block_NLM pHalfImg;
	LPImgPyramid_Block_NLM pFullImg;
	MInt32  topline;
	MInt32  botline;
...
MVoid thread_Img_UpScale_C1(MVoid* pParam)
{
	LpImg_Up_Scale_St Img_Up2_sturct = (LpImg_Up_Scale_St)pParam;
	MHandle hMemMgr = Img_Up2_sturct->hMemMgr;
	LPImgPyramid_Block_NLM pHalfImg = Img_Up2_sturct->pHalfImg;
	LPImgPyramid_Block_NLM pFullImg = Img_Up2_sturct->pFullImg;
