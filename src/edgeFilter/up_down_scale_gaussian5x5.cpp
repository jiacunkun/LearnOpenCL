...
MInt32 Img_Guass5x5_Down2_u8(MHandle hMemMgr, MHandle mcvParallelMonitor, LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pDstImg, MUInt16** tmpBuf)
{
	...
	MInt32  lSrcWidth = 0;
	MInt32  lDstH = 0;
	MUInt16 *tmpBuf0[16] = { MNull };
	MBool isExternalBuf = tmpBuf == MNull ? MFalse : MTrue;
	MInt32 lTaskNum = 1;
	MInt32 lnum;
...
#ifdef MCV_MULTI_THREAD //TODO: 该处外部没有传入多线程指针，导致无法正常运行
	...
	{
		gaussPyrdown_u8_range(hMemMgr, pSrcImg, pDstImg, tmpBuf[0], 0, lDstH);
	}
