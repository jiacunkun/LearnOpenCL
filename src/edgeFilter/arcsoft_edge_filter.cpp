...
arcsoft_edge_filter::arcsoft_edge_filter()
{
    m_hMemMgr = MNull;
    ...
    for (int i = 0; i < MAX_PYRAMID_LAYER; i++)
    {
        ...
        MMemSet(&m_pGuidePyrGau[i], 0, sizeof(ASVLOFFSCREEN));
        MMemSet(&m_pMeanA[i], 0, sizeof(ASVLOFFSCREEN));
        MMemSet(&m_pMeanB[i], 0, sizeof(ASVLOFFSCREEN));
    }
}
...
MInt32 arcsoft_edge_filter::init(MHandle hMemMgr, MHandle mcvParallelMonitor, MInt32 lWidth, MInt32 lHeight, MInt32 lStride, MInt32 lPryLayer)
{
    START_TIME;
    MInt32 lRet = 0;
    m_hMemMgr = hMemMgr;
    ...
    for (int i = 1; i < m_lPryLayer; i++)
    {
        ...
        CHECK_ERROR(lRet);
        lRet = AllocLPASVLOFFSCREEN(hMemMgr, &m_pGuidePyrGau[i], newWidth, newHeight, newStride, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);
        ...
        RectLPASVLOFFSCREEN(&m_pGuidePyrGau[i], &m_pGuidePyrGauInner[i], &Roi);
...
MVoid arcsoft_edge_filter::release()
{
    START_TIME;
    for (int i = 1; i < m_lPryLayer; i++)
    {
        FreeLPASVLOFFSCREEN(m_hMemMgr, &m_pSrcPyrGau[i]);
        FreeLPASVLOFFSCREEN(m_hMemMgr, &m_pGuidePyrGau[i]);
...
MInt32 arcsoft_edge_filter::run(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pGuided, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
    ...
    m_pGuidePyrGauInner[0] = *pGuided;
    lRet = BuildGauPyrs_u8(m_hMemMgr, m_mcvParallelMonitor, pSrcImg, m_pSrcPyrGauInner, m_lPryLayer, MNull);
    lRet = BuildGauPyrs_u8(m_hMemMgr, m_mcvParallelMonitor, pGuided, m_pGuidePyrGauInner, m_lPryLayer, MNull);
    ...
    for (MInt32 i = m_lPryLayer - 1; i > 1; i--)
    {
        if (pValue[i] > 0.0)
        {
            ...
            FillExpandPixels(&m_pGuidePyrGau[i], m_lExpandSize);
        ...
        ImgSubImg_add128_u8(&m_pDstPyrGauInner[i], &m_pSrcPyrGauInner[i], &m_pDstPyrGauInner[i]);
    ...
    if (lScale == 4)
    {
        ...
        //Img_Guass5x5_Up2_u8(m_hMemMgr, &m_pMeanAInner[1], &m_pMeanAInner[0]);
        //Img_Guass5x5_Up2_u8(m_hMemMgr, &m_pMeanBInner[1], &m_pMeanBInner[0]);
        Img_Guass3x3_Up2_u8(m_hMemMgr, m_mcvParallelMonitor, &m_pMeanAInner[i + 1], &m_pMeanAInner[i]);
        Img_Guass3x3_Up2_u8(m_hMemMgr, m_mcvParallelMonitor, &m_pMeanBInner[i + 1], &m_pMeanBInner[i]);
    ...
    if (pValue[0] > 0.0)
    {
        lRet = AllocLPASVLOFFSCREEN(m_hMemMgr, &m_pMeanAInner[i], m_lWidth, m_lHeight, m_lWidth, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);
        lRet = AllocLPASVLOFFSCREEN(m_hMemMgr, &m_pMeanBInner[i], m_lWidth, m_lHeight, m_lWidth, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);

        //Img_Guass5x5_Up2_u8(m_hMemMgr, &m_pMeanAInner[1], &m_pMeanAInner[0]);
        //Img_Guass5x5_Up2_u8(m_hMemMgr, &m_pMeanBInner[1], &m_pMeanBInner[0]);
        Img_Guass3x3_Up2_u8(m_hMemMgr, m_mcvParallelMonitor, &m_pMeanAInner[1], &m_pMeanAInner[0]);
        Img_Guass3x3_Up2_u8(m_hMemMgr, m_mcvParallelMonitor, &m_pMeanBInner[1], &m_pMeanBInner[0]);
        ...
        FreeLPASVLOFFSCREEN(m_hMemMgr, &m_pMeanAInner[i]);
        FreeLPASVLOFFSCREEN(m_hMemMgr, &m_pMeanBInner[i]);
    }
...
MInt32 arcsoft_edge_filter::run(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
...
MInt32 arcsoft_edge_filter::run2(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pGuided, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
    ...
    //0层
    MInt32 i = 0;
    mat_write255(pDstImg->i32Height, pDstImg->pi32Pitch[0], CV_8UC1, pDstImg->ppu8Plane[0], "srcDst0.png", 1.0);

    if (pValue[0] > 0.0)
#if 1
    {
        int newWidth = m_lWidth + m_lExpandSize * 2;
        int newHeight = m_lHeight + m_lExpandSize * 2;

        lRet = AllocLPASVLOFFSCREEN(m_hMemMgr, &m_pGuidePyrGau[i], newWidth, newHeight, newWidth, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);
        lRet = AllocLPASVLOFFSCREEN(m_hMemMgr, &m_pSrcPyrGau[i], newWidth, newHeight, newWidth, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);
        lRet = AllocLPASVLOFFSCREEN(m_hMemMgr, &m_pMeanB[i], newWidth, newHeight, newWidth, ASVL_PAF_GRAY);
        CHECK_ERROR(lRet);

        Rect Roi = { m_lExpandSize, m_lExpandSize, m_lWidth, m_lHeight };
        RectLPASVLOFFSCREEN(&m_pSrcPyrGau[i], &m_pSrcPyrGauInner[i], &Roi);
        RectLPASVLOFFSCREEN(&m_pGuidePyrGau[i], &m_pGuidePyrGauInner[i], &Roi);
        RectLPASVLOFFSCREEN(&m_pMeanB[i], &m_pMeanBInner[i], &Roi);

        CopyY(&m_pSrcPyrGauInner[i], &m_pDstPyrGauInner[i]);
        CopyY(&m_pGuidePyrGauInner[i], pGuided);

        FillExpandPixels(&m_pSrcPyrGau[i], m_lExpandSize);
        FillExpandPixels(&m_pGuidePyrGau[i], m_lExpandSize);
        ...
        CopyY(&m_pDstPyrGauInner[i], &m_pMeanBInner[i]);
...
#else
    {
        MFloat fIntensity = pValue[0] * 10;
        NLMeans2(m_hMemMgr, m_mcvParallelMonitor, fIntensity).Run(&m_pDstPyrGauInner[i], &m_pDstPyrGauInner[i], MNull, fIntensity, 8);
    }
#endif
...
MInt32 arcsoft_edge_filter::runWithNLM(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pGuided, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
    ...
    if (pValue[0] > 0.0)
    {
        ...
        obj.Run(&m_pDstPyrGauInner[i], &m_pDstPyrGauInner[i], MNull, fIntensity, 8);
    }
...
MInt32 arcsoft_edge_filter::runB(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
...
MInt32 arcsoft_edge_filter::runValue(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pDstImg, MFloat* pValue, MInt32 lScale)
{
    START_TIME;
    MInt32 lRet = 0;
    ...
    for (MInt32 i = m_lPryLayer - 1; i >= 1; i--)
    {
        if (pValue[i] > 0.0)
        {
            ...
            sprintf(srcFilename, "src[%d].png", i);
            sprintf(dstFilename, "dst[%d].png", i);
#endif
            mat_write255(m_pDstPyrGau[i].i32Height, m_pDstPyrGau[i].pi32Pitch[0], CV_8UC1, m_pDstPyrGau[i].ppu8Plane[0], srcFilename, 1.0);
            ValueFilter(m_hMemMgr, m_mcvParallelMonitor, &m_pDstPyrGauInner[i], &m_pDstPyrGauInner[i], pValue[i]);
            mat_write255(m_pDstPyrGau[i].i32Height, m_pDstPyrGau[i].pi32Pitch[0], CV_8UC1, m_pDstPyrGau[i].ppu8Plane[0], dstFilename, 1.0);
        }
        else
        {
            CopyY(&m_pDstPyrGauInner[i], &m_pSrcPyrGauInner[i]);
    ...
    if (pValue[0] > 0.0)
    {
        ...
        mat_write255(m_pDstPyrGauInner[i].i32Height, m_pDstPyrGauInner[i].pi32Pitch[0], CV_8UC1, m_pDstPyrGauInner[i].ppu8Plane[0], "src0.png", 1.0);
        ValueFilter(m_hMemMgr, m_mcvParallelMonitor, &m_pDstPyrGauInner[i], &m_pDstPyrGauInner[i], pValue[i]);
        mat_write255(m_pDstPyrGauInner[i].i32Height, m_pDstPyrGauInner[i].pi32Pitch[0], CV_8UC1, m_pDstPyrGauInner[i].ppu8Plane[0], "dst0.png", 1.0);
    }
...
MInt32 arcsoft_edge_filter::CalcDetailMask()
{
    START_TIME;
...
MVoid  arcsoft_edge_filter::FillExpandPixels(LPASVLOFFSCREEN pSrcDst, MInt32 lExpandSize)
{
    START_TIME;
...
MVoid arcsoft_edge_filter::ImgAddDiff_sub128_u8(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pDiffImg, LPASVLOFFSCREEN pDstImg)
{
    START_TIME;
    MInt32 lHeight = pSrcImg->i32Height;
...
MVoid arcsoft_edge_filter::ImgSubImg_add128_u8(LPASVLOFFSCREEN pSrcImg, LPASVLOFFSCREEN pSubImg, LPASVLOFFSCREEN pDstImg)
{
    START_TIME;
    MInt32 lHeight = pSrcImg->i32Height;
