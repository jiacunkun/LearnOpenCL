...
{                                                                            \
    ...
    NLMeans16::NLMeans16(MHandle hMemMgr, MHandle mcvParallelMonitor, MFloat fIntensity)
    {
        m_hMemMgr = hMemMgr;
    ...
    /// @param pTable   [out]
    /// @param lSize    [in]    权重的最大值, 默认为1024x9+1, 搜索范围为3x3
    /// @return
    MVoid NLMeans16::MakeDivTable(MInt32 *pTable, MInt32 lSize)
    {
    ...
    /// @return
    MVoid NLMeans16::MakeWeightMap(MInt32 *pTable, MFloat fVar, MInt32 lMaxNum)
    {
        //    printf("fVar = %f\n", fVar);
    ...
    /// @param fIntensity
    /// @return
    MVoid NLMeans16::UpdateWeightMap(MFloat fIntensity)
    {
        NLMeans16::MakeWeightMap(m_pMap, fIntensity, 16 * 50);
    }
    ...
    /// @param l_add        [in]
    /// @return
    MVoid NLMeans16::ProcessPointBround(MInt16 *pCurPoint,
                                        MInt16 *pPrePoint,
                                        MInt16 *pNexPoint,
                                        MInt16 *pDstPoint,
                                        MInt32 *pMap,
    ...
    /// @param pInvMap
    /// @return
    MVoid NLMeans16::ProcessPoint(MInt16 *pCurPoint,
                                  MInt16 *pPrePoint,
                                  MInt16 *pNexPoint,
                                  MInt16 *pDstPoint,
                                  MInt32 *pMap,
    ...
    /// @param lPitch       [in]
    /// @return 匹配距离和
    MInt32 NLMeans16::GetBlockDiff(MInt16 *pCurBlock, MInt16 *pNeiBlock, MInt32 lPitch)
    {
        MInt32 lDif = 0;
        MInt32 lPDif = 0;

        MInt32 max_diff = 49*4;

        lPDif = ABS(pCurBlock[ 0 ] - pNeiBlock[ 0 ]);
        lPDif = MIN(max_diff, lPDif);
        lDif += lPDif;
    ...
    /// @param lW           [in]
    /// @return
    MVoid NLMeans16::AddBlockSum(MInt16 *pNeiBlock, MInt32 lPitch, MInt32 *lSumWei, MInt32 lW)
    {
        lSumWei[ 0 ] += lW;
    ...
    /// @param pMap         [in]
    /// @return
    MVoid NLMeans16::AddBlockSumByNei(MInt16 *pCurBlock,
                                      MInt16 *pNeiBlock,
                                      MInt32 lPitch,
                                      MInt32 *lSumWei,
                                      MInt32 *pMap,
...
#define MAX_CLAMP 1020
    ...
    /// @param pInvMap      [in]
    /// @return
    MVoid NLMeans16::GetBlockResult(MInt16 *pDstBlock,
                                    MInt32 lPitch,
                                    MInt32 *lSumWei,
                                    MInt32 *pInvMap)
    {
        ...
        lSumWei++;
#ifdef USE_NEON_NLM
        int32x4_t V_const_max = vdupq_n_s32(MAX_CLAMP);
        MInt32 resData[4];
        ...
        tmpdata = vminq_s32(tmpdata, V_const_max);
        ...
        pDstBlock[ 0 ] = pDstBlock[ 0 ] > MAX_CLAMP ? MAX_CLAMP : pDstBlock[ 0 ];
        pDstBlock[ 1 ] = pDstBlock[ 1 ] > MAX_CLAMP ? MAX_CLAMP : pDstBlock[ 1 ];
        pDstBlock[ 2 ] = pDstBlock[ 2 ] > MAX_CLAMP ? MAX_CLAMP : pDstBlock[ 2 ];
        pDstBlock[ 3 ] = pDstBlock[ 3 ] > MAX_CLAMP ? MAX_CLAMP : pDstBlock[ 3 ];
        pDstBlock += lPitch;
        lSumWei += 4;
    ...
    /// @param pInvMap
    /// @return
    MVoid NLMeans16::ProcessBlock4x4(MInt16 *pCurLine,
                                     MInt16 *pPreLine,
                                     MInt16 *pNexLine,
                                     MInt16 *pDstLine,
                                     MInt32 *pMap,
                                     ...
                                     MInt32 difScale0)
    {
        ...
        // 领域8个位置
        AddBlockSumByNei(pCurLine, pCurLine - 1, lPitch, lSumWei, pMap, difScale0);
        AddBlockSumByNei(pCurLine, pCurLine + 1, lPitch, lSumWei, pMap, difScale0);

        AddBlockSumByNei(pCurLine, pPreLine - 1, lPitch, lSumWei, pMap, difScale0);
        AddBlockSumByNei(pCurLine, pPreLine, lPitch, lSumWei, pMap, difScale0);
        AddBlockSumByNei(pCurLine, pPreLine + 1, lPitch, lSumWei, pMap, difScale0);

        AddBlockSumByNei(pCurLine, pNexLine - 1, lPitch, lSumWei, pMap, difScale0);
        AddBlockSumByNei(pCurLine, pNexLine, lPitch, lSumWei, pMap, difScale0);
        AddBlockSumByNei(pCurLine, pNexLine + 1, lPitch, lSumWei, pMap, difScale0);
...
//		LOGD("lSumWei[0] = %d\n", lSumWei[0]);
    ...
    /// @param plW
    /// @param pSharedBuffer
    void NLMeans16::GetBlockDiff4Neon(MInt16 *pCurLine,
                                      MInt16 *pPreLine,
                                      MInt16 *pNexLine,
                                      MInt32 lPitch,
                                      MInt32 *pMap,
    ...
    //#endif
        ...
        MInt16 *pCurLine_Left = pCurLine - 1;
        MInt16 *pPreLine_Left = pPreLine - 1;
        MInt16 *pNextLine_Left = pNexLine - 1;
        ...
        for(row = 0; row < 4; row++)
        {
            ...
            //计算4 , 6
            int16x8_t V_curLeft16x8 = vld1q_s16(pCurLine_Left);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_curLeft8x8 = vmovn_u16(V_curLeft16x8);

            uint8x8_t V_cur8x8 = vtbl1_u8(V_curLeft8x8, V_tbl_mid); // todo:目前没有对16bit的查表，目前用的是强转
...
//            uint8x8_t V_preLeft8x8 = vld1_u8(pPreLine_Left);
            uint16x8_t V_preLeft16x8 = vld1q_s16(pPreLine_Left);
            V_curLeft16x8 = vshrq_n_u16(V_preLeft16x8, 2);
            uint8x8_t V_preLeft8x8 = vmovn_u16(V_curLeft16x8);
...
//            uint8x8_t V_nextLeft8x8 = vld1_u8(pNextLine_Left);
            uint16x8_t V_nextLeft16x8 = vld1q_s16(pNextLine_Left);
            V_curLeft16x8 = vshrq_n_u16(V_nextLeft16x8, 2);
            uint8x8_t V_nextLeft8x8 = vmovn_u16(V_curLeft16x8);
    ...
    /// @param plW
    /// @param pSharedBuffer
    void NLMeans16::GetBlockDiff8Neon(MInt16 *pCurLine,
                                      MInt16 *pPreLine,
                                      MInt16 *pNexLine,
                                      MInt32 lPitch,
                                      MInt32 *pMap,
    ...
    //#endif
        ...
        for(row = 0; row < 4; row++)    //4x4 , line 4
        {
            ...
            //计算4 , 6
//            uint16x8_t V_curLeft8x8 = vld1q_u16(pCurLine - 1);
//            uint16x8_t V_cur8x8 = vld1q_u16(pCurLine);
//            uint16x8_t V_curRight8x8 = vld1q_u16(pCurLine + 1);

            // todo: 16bit读取后，强转8bit
            uint16x8_t V_curLeft16x8 = vld1q_s16(pCurLine - 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_curLeft8x8 = vmovn_u16(V_curLeft16x8);
            V_curLeft16x8 = vld1q_s16(pCurLine);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_cur8x8 = vmovn_u16(V_curLeft16x8);
            V_curLeft16x8 = vld1q_s16(pCurLine + 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_curRight8x8 = vmovn_u16(V_curLeft16x8);


            V_curLeft16x8 = vreinterpretq_s16_u16(vmovl_u8(V_curLeft8x8));
...
//            V_diffSum16x8 = vshrq_n_s16(V_diffSum16x8, 2);
...
//            V_diffSum16x8 = vshrq_n_s16(V_diffSum16x8, 2);
            ...
            //计算1  2  3
//            uint16x8_t V_preLeft8x8 = vld1q_u16(pPreLine - 1);
//            uint16x8_t V_pre8x8 = vld1q_u16(pPreLine);
//            uint16x8_t V_preRight8x8 = vld1q_u16(pPreLine + 1);

            // todo: 16bit读取后，强转8bit
            V_curLeft16x8 = vld1q_s16(pPreLine - 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_preLeft8x8 = vmovn_u16(V_curLeft16x8);

            V_curLeft16x8 = vld1q_s16(pPreLine);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_pre8x8 = vmovn_u16(V_curLeft16x8);

            V_curLeft16x8 = vld1q_s16(pPreLine + 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_preRight8x8 = vmovn_u16(V_curLeft16x8);
...
//            V_diffSum16x8 = vshrq_n_s16(V_diffSum16x8, 2);
            ...
            //计算7  8  9
//            uint16x8_t V_nextLeft8x8 = vld1q_u16(pNexLine - 1);
//            uint16x8_t V_next8x8 = vld1q_s16(pNexLine);
//            uint16x8_t V_nextRight8x8 = vld1q_s16(pNexLine + 1);

            // todo: 16bit读取后，强转8bit
            V_curLeft16x8 = vld1q_s16(pNexLine - 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_nextLeft8x8 = vmovn_u16(V_curLeft16x8);

            V_curLeft16x8 = vld1q_s16(pNexLine);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_next8x8 = vmovn_u16(V_curLeft16x8);

            V_curLeft16x8 = vld1q_s16(pNexLine + 1);
            V_curLeft16x8 = vshrq_n_u16(V_curLeft16x8, 2);
            uint8x8_t V_nextRight8x8 = vmovn_u16(V_curLeft16x8);
...
//            V_diffSum16x8 = vshrq_n_s16(V_diffSum16x8, 2);
    ...
    /// @param lSumWei
    /// @param plW
    void NLMeans16::AddBlockSumByNei3x3Neon(MInt16 *pCurLine,
                                            MInt16 *pPreLine,
                                            MInt16 *pNexLine,
                                            MInt32 lPitch,
                                            MInt32 *lSumWei,
                                            MInt32 *plW)
    {
        ...
        for(row = 0; row < 4; row++, pRoot += 4)
        {
            ...
            //计算4 , 6
            uint16x4_t V_curLeft8x8 = vld1_s16(pCurLine - 1);
            uint16x4_t V_curRight8x8 = vld1_s16(pCurLine + 1);
            int32x4_t V_curLeft32x4 = vmovl_s16((vreinterpret_s16_u16((V_curLeft8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_curLeft32x4, curLeftlW));
            int32x4_t V_curRight32x4 = vmovl_s16((vreinterpret_s16_u16((V_curRight8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_curRight32x4, curRightlW));

            //计算1 , 2 , 3
            uint16x4_t V_preLeft8x8 = vld1_s16(pPreLine - 1);
            uint16x4_t V_pre8x8 = vld1_s16(pPreLine);
            uint16x4_t V_preRight8x8 = vld1_s16(pPreLine + 1);
            int32x4_t V_preLeft32x4 = vmovl_s16((vreinterpret_s16_u16((V_preLeft8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_preLeft32x4, preLeftlW));
            int32x4_t V_pre32x4 = vmovl_s16((vreinterpret_s16_u16((V_pre8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_pre32x4, prelW));
            int32x4_t V_preRight32x4 = vmovl_s16((vreinterpret_s16_u16((V_preRight8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_preRight32x4, preRightlW));

            //计算7 , 8 , 9
            uint16x4_t V_nextLeft8x8 = vld1_s16(pNexLine - 1);
            uint16x4_t V_next8x8 = vld1_s16(pNexLine);
            uint16x4_t V_nextRight8x8 = vld1_s16(pNexLine + 1);
            int32x4_t V_nextLeft32x4 = vmovl_s16((vreinterpret_s16_u16((V_nextLeft8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_nextLeft32x4, nextLeftlW));
            int32x4_t V_next32x4 = vmovl_s16((vreinterpret_s16_u16((V_next8x8))));
            V_sumWei = vaddq_s32(V_sumWei, vmulq_n_s32(V_next32x4, nextlW));
            int32x4_t V_nextRight32x4 = vmovl_s16((vreinterpret_s16_u16((V_nextRight8x8))));
    ...
    /// @param pDstLine
    /// @param pInvMap
    void NLMeans16::ProcessBlock4x4Neon(MInt16 *pCurLine,
                                      MInt16 *pPreLine,
                                      MInt16 *pNexLine,
                                      MInt32 *lSumWei,
                                      MInt32 lPitch,
                                      ...
                                      MInt32 *pInvMap)
    {
        ...
        AddBlockSumByNei3x3Neon(pCurLine, pPreLine, pNexLine, lPitch, lSumWei, lW);
    ...
    /// @param pInvMap
    /// @return
    MVoid NLMeans16::ProcessLinesBroundMain(MInt16 *pCurLine,
                                            MInt16 *pPreLine,
                                            MInt16 *pDstLine,
                                            MInt32 lWidth,
                                            MInt32 *pMap,
    ...
    /// @param pInvMap
    /// @return
    MVoid NLMeans16::ProcessLines1Main(MInt16 *pCurLine,
                                       MInt16 *pPreLine,
                                       MInt16 *pNexLine,
                                       MInt16 *pDstLine,
                                       MInt32 lWidth,
    ...
    /// @param pInvMap
    /// @return
    MVoid NLMeans16::ProcessLines4Main(MInt16 *pCurLine,
                                       MInt16 *pPreLine,
                                       MInt16 *pNexLine,
                                       MInt16 *pDstLine,
                                       MInt32 lWidth,
                                       ...
                                       MInt32 shadeWidth)
    {
        ...
        /// 最左边一个点
        {
            ...
            lShift += lPitch;
            lDstShift += lDstPitch;
            ProcessPointBround(pCurLine + lShift, pPreLine + lShift, pNexLine + lShift, pDstLine + lShift, pMap, pInvMap, 1);
            lShift += lPitch;
            lDstShift += lDstPitch;
            ProcessPointBround(pCurLine + lShift, pPreLine + lShift, pNexLine + lShift, pDstLine + lShift, pMap, pInvMap, 1);
            lShift += lPitch;
        ...
        for(; x < lWidth - 1; x++)
        {
            ...
            lShift += lPitch;
            lDstShift += lDstPitch;
            ProcessPoint(pCurLine + lShift, pPreLine + lShift, pNexLine + lShift, pDstLine + lShift, pMap, pInvMap);
            lShift += lPitch;
            lDstShift += lDstPitch;
            ProcessPoint(pCurLine + lShift, pPreLine + lShift, pNexLine + lShift, pDstLine + lShift, pMap, pInvMap);
            lShift += lPitch;
    ...
    /// @return
    MInt32 NLMeans16::Process(ImageInfo<MInt16> *pSrcImg,
                              ImageInfo<MInt16> *pDstImg,
                              ImageInfo<MByte>  *pDnShade,
                              MInt32 lTopLine,
                              MInt32 lBotLine,
                              MFloat fIntensity /*= -1*/)
    {
        ...
        MInt32 lPitch = pSrcImg->lStride;
        MInt32 lDstPitch = pDstImg->lStride;
        MInt16 *pSrcData = pSrcImg->pData;
        ...
        for(; y < lBlock_Bot; y += 4)
        {
            ...
            ProcessLines4Main(pCurLine, pPreLine, pNexLine, pDstLine, lWidth, lPitch, lDstPitch, pMap, pInvMap, pDnShadeLine, shadeWidth);
        }
    ...
    /// @param pDstImg      [in]
    /// @return
    MInt32 NLMeans16::Run(ImageInfo<MInt16> *pSrcImg,
                          ImageInfo<MInt16> *pDstImg,
                          ImageInfo<MByte> *pDnShade,
                          MFloat fIntensity,
                          MInt32 nThreadCount)
    {
        ...
        if( isRunThreads )
        {
            ...
            auto func_lamda = [](MVoid *HParam) -> MVoid
            {
                ...
                ImageInfo<MInt16> *pSrcImg = SG_NLM_sturct->pSrcImg;
                ImageInfo<MInt16> *pDstImg = SG_NLM_sturct->pDstImg;
                ImageInfo<MByte>  *pDnShade = SG_NLM_sturct->pDnShade;
                MFloat fIntensity = SG_NLM_sturct->fIntensity;
                MInt32 lret = MOK;

                NLMeans16 *obj = ( NLMeans16 * ) SG_NLM_sturct->obj;
