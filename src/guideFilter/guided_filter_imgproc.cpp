...
static MRESULT splitChannels_U8C1(MHandle parEngine, MInt32 taskNum,
...
static MRESULT splitChannels_U8C3(MHandle parEngine, MInt32 taskNum, 
	const MByte* pSrc, MInt32 srcPitch, MInt32 height, MInt32 width,
	...
	MByte* pDst, MInt32 dstPitch, MInt32 height, MInt32 width, MInt32 lStartRow, MInt32 lEndRow)
{
...
#ifdef __ARM_NEON__
	float32x4_t v_DstLow, v_DstHigh, v_constant;
	uint16x4_t v_tmp1, v_tmp2;
	uint8x8_t v_dst;
	v_constant = vdupq_n_f32(0.5f);
#endif
	...
	for (i = lStartRow; i < lEndRow; ++i)
	{
		...
		for (; j < width - 7; j += 8)
		{
			...
			v_DstLow = vaddq_f32(v_DstLow, v_constant);
			v_DstHigh = vaddq_f32(v_DstHigh, v_constant);
		...
		for (; j < width; ++j)
		{
			ANYTYPE_CAST_8U(pDCn[j] + 0.5f, pD[j]);
		}
	}
}
...
static MRESULT MergeChannelsAndConvertToDstType_U8C1(MHandle parEngine, MInt32 taskNum,
...
static MRESULT MergeChannelsAndConvertToDstType_U8C3(MHandle parEngine, MInt32 taskNum,
...
{
	MInt32 srcStep = srcPitch, dstStep = dstPitch / sizeof(MInt32);
	...
	MFloat *pDst, MInt32 dstPitch, MInt32 height, MInt32 width, MInt32 radius)
{
	...
	{
		MInt32 *boxSumBuf = (MInt32 *)MMemAlloc(MNull, (width + radius * 2 + 1 + 100) * sizeof(MInt32));
		if (!boxSumBuf) return MERR_NO_MEMORY;
...
static MRESULT CalcCovarInv_F32C1(MHandle parEngine, MInt32 taskNum, MFloat **pCovar,
	MInt32 pitch, MInt32 height, MInt32 width, MFloat **pCovarInv)
{
	if (parEngine)
	{
...
MVoid thread_CalcAlpha_F32(MVoid *info)
{
...
MVoid thread_CalcBeta_F32(MVoid *info)
{
...
MVoid thread_ApplyTransform_F32U8(MVoid *info)
{
...
MVoid thread_ImgBilinearResize_F32C1(MVoid *info)
{
	...
	MInt32 dstWidth, MInt32 dstHeight, MInt32 taskNum, BilinearResize_Param *pParam)
{
	...
	buf_size = dstWidth * 2 * taskNum * sizeof(MInt16)
		+ sizeof(MInt32) 
...
MVoid thread_ImgBilinearResize_U8C1(MVoid *info)
{
