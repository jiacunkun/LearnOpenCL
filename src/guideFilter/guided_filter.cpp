	...
	const MByte *guidanceData, MInt32 pitch, MInt32 height, MInt32 width, MInt32 channels)
{
	...
	for (c1 = 0; c1 < channels; ++c1)
	{
		...
		res = BoxFilter_U8C1toF32C1(threadEngine, taskNum, pAgfInfo->pGuidanceCnOri[c1], pitch_sc_uint8,
			pAgfInfo->pGuidanceCnMean[c1], pitch_sc_float32, height, width, gfRadius);
		CHECK_ERROR(res);
		for (c2 = c1; c2 < channels; ++c2)
		{
			// calculate corrGuidance
			res = DotMultiply_U8C1toU16C1(threadEngine, taskNum, pAgfInfo->pGuidanceCnOri[c1], pAgfInfo->pGuidanceCnOri[c2],
				pitch_sc_uint8, pAgfInfo->pCorrGuidance[total], pitch_sc_uint16, height, width);
			CHECK_ERROR(res);
			// mean filter for corrGuidance -> corrGuidanceMean
			res = BoxFilter_U16C1toF32C1(threadEngine, taskNum, pAgfInfo->pCorrGuidance[total], pitch_sc_uint16,
...
MRESULT guidedFilter_Filtering(MHandle threadEngine, MInt32 taskNum, ArcGuidedFilter* pAgfInfo,
                               const MByte *srcData, MByte *dstData, MInt32 pitch, MInt32 height, MInt32 width, MInt32 channels)
{
	...
	for (c1 = 0; c1 < channels; ++c1)
	{
		...
		res = BoxFilter_U8C1toF32C1(threadEngine, taskNum,
		...
		for (c2 = 0; c2 < guidanceCnNum; ++c2)
		{
			...
			res = DotMultiply_U8C1toU16C1(threadEngine, taskNum, 
			...
			res = BoxFilter_U16C1toF32C1(threadEngine, taskNum, pAgfInfo->pCorrGuidanceSrc[idx], pitch_sc_uint16,
	...
	const MByte *guidanceData, MInt32 pitch, MInt32 height, MInt32 width, MInt32 channels)
{
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement ImgBilinearResize_U8C1 consume time = %d\r\n", lTime);
#endif
	}
	...
	for (c1 = 0; c1 < channels; ++c1)
	{
		...
		res = BoxFilter_U8C1toF32C1(threadEngine, taskNum, pAgfInfo->pGuidanceCnSub[c1], pitchSub_sc_uint8, 
			pAgfInfo->pGuidanceCnMean[c1], pitchSub_sc_float32, heightSub, widthSub, gfRadiusSub);
		CHECK_ERROR(res);
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement BoxFilter_U8C1toF32C1 consume time = %d\r\n", lTime);
#endif
		for (c2 = c1; c2 < channels; ++c2)
		{
#if defined(_ARM_TIME_)
			...
			CHECK_ERROR(res);
			// mean filter for corrGuidance -> corrGuidanceMean
			res = BoxFilter_U16C1toF32C1(threadEngine, taskNum, pAgfInfo->pCorrGuidance[total], pitchSub_sc_uint16,
				pAgfInfo->pCorrGuidanceMean[total], pitchSub_sc_float32, heightSub, widthSub, gfRadiusSub);
			CHECK_ERROR(res);
			total++;
#if defined(_ARM_TIME_)
			END_PROFILE(lTime);
			PrintfB(6, "VNS", "aveDoEnhancement DotMultiply_U8C1toU16C1 consume time = %d\r\n", lTime);
#endif
		}
	}
...
#if defined(_ARM_TIME_)
			END_PROFILE(lTime);
			PrintfB(6, "VNS", "aveDoEnhancement CalcVarAddEpsilon_F32C1 consume time = %d\r\n", lTime);
#endif
		}
		else
		{
...
#if defined(_ARM_TIME_)
			END_PROFILE(lTime);
			PrintfB(6, "VNS", "aveDoEnhancement CalcCovar_F32C1 consume time = %d\r\n", lTime);
#endif
		}
	}
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement CalcCovarInv_F32 consume time = %d\r\n", lTime);
#endif
	}
...
MRESULT fastGuidedFilter_Filtering(MHandle threadEngine, MInt32 taskNum, ArcGuidedFilter* pAgfInfo,
                                   const MByte *srcData, MByte *dstData, MInt32 pitch, MInt32 height, MInt32 width, MInt32 channels)
{
	MHandle hMemMgr = pAgfInfo->hMemMgr;
	...
	for (c1 = 0; c1 < channels; ++c1)
	{
		...
		for (c2 = 0; c2 < guidanceCnNum; ++c2)
		{
			...
			res = BoxFilter_U16C1toF32C1(threadEngine, taskNum,
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement CalcAlpha_F32 consume time = %d\r\n", lTime);
#endif
	}
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement CalcBeta_F32 consume time = %d\r\n", lTime);
#endif
	}
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement BoxFilter_F32C1 consume time = %d\r\n", lTime);
#endif
		for (c2 = 0; c2 < guidanceCnNum; ++c2)
		{
			idx = c1 * channels + c2;
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement ImgBilinearResize_F32C1 consume time = %d\r\n", lTime);
#endif
		for (c2 = 0; c2 < guidanceCnNum; ++c2)
		{
			idx = c1 * channels + c2;
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement ApplyTransform_F32U8 consume time = %d\r\n", lTime);
#endif
	}
...
#if defined(_ARM_TIME_)
		END_PROFILE(lTime);
		PrintfB(6, "VNS", "aveDoEnhancement MergeChannelsAndConvertToDstType consume time = %d\r\n", lTime);
#endif
	}
